...
#' @title Define event based processes that outputs individual level data to a csv
#' @description defines processes for events that can be scheduled in the future
#' @param variables list of variables in the model
#' @param events a list of events in the model
#' @param parameters the model parameters
...
#' @param renderer the model rendering object
#' @noRd
attach_verbose_event_listeners <- function(
  events,
  variables,
  parameters,
  correlations,
  ...
  if (parameters$bednets == 1) {
    events$throw_away_net$add_listener(
      throw_away_nets_verbose(variables, parameters)
    )
  }
  ...
  if (!is.null(events$mass_pev)) {
    # set up distribution
    events$mass_pev$add_listener(
      create_verbose_mass_pev_listener(
        variables,
        events,
        parameters,
        correlations
