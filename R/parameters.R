...
#'
#' * init_ib  - the initial pre-erythrocytic immunity (p.f only); default = 0
#' * init_id  - the initial acquired immunity to lm-detectability (p.f) or anti-parasite immunity (p.v); default = 0
#' * init_idm - the initial maternal anti-parasite immunity at birth (p.v only); default = 0
#' * init_ica - the initial acquired immunity from clinical disease; default = 0
#' * init_icm - the initial maternal immunity from clinical disease at birth; default = 0
#' * init_iva - the initial acquired immunity from severe disease (p.f only); default = 0
#' * init_ivm - the initial immunity from severe disease at birth (p.f only); default = 0
#'
...
#' * ud - period in which immunity to detectability (p.f) or antiparasite immunity (p.v) cannot be boosted; default = 9.44512
...
#'
#' * f - hypnozoite relapse rate; default = 0.024
#' * gammal - hypnozoite clearance rate (rate of an individual clearing a single hypnozoite batch); default = 0.0026
...
#' * hypnozoite_prevalence_rendering_min_ages - the minimum ages for hypnozoite prevalence outputs (p.v only); default = turned off
#' * hypnozoite_prevalence_rendering_max_ages - the corresponding max ages; default = turned off
#'
#' miscellaneous:
#'
...
get_parameters <- function(overrides = list(), parasite = "falciparum") {
  ...
  parameters <- c(
    ...
    list(
      ...
      ib0   = 43.9,
      kb    = 2.16,
      # asymptomatic immunity to lm detection parameters
      fd0   = 0.007055,
      ad    = 21.9 * 365,
      gammad= 4.8183,
      d1    = 0.160527,
