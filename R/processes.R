...
#' @title Define model processes which output individual level information
#' @description
#' create_processes, defines the functions which describe how each individual's
#' variables change over time.
#'
...
#' lagged transmission lists (default: 1)
#' @noRd
create_verbose_processes <- function(
    renderer,
    variables,
    events,
    parameters,
  ...
  progression_outcome <- CompetingOutcome$new(
    targeted_process = function(timestep, target){
      progression_outcome_process_verbose(timestep, target, variables, parameters, renderer)
    },
    size = parameters$human_population
  )
  ...
  processes <- c(
    processes,
    biting_process = create_verbose_biting_process(
      renderer,
      solvers,
      models,
      variables,
  ...
  processes <- c(
    processes,
    progression_process = create_verbose_progression_rates_process(
      parameters,
      variables,
      progression_outcome
    ),
  ...
  if (!is.null(parameters$pev_epi_coverage)) {
    processes <- c(
      processes,
      epi_pev_process = create_verbose_epi_pev_process(
        variables,
        events,
        parameters,
        correlations,
  ...
  if (parameters$bednets) {
    processes <- c(
      processes,
      distribute_nets_process = distribute_nets_verbose(
        variables,
        events$throw_away_net,
        parameters,
        correlations
  ...
  processes <- c(
    processes,
    state_process = render_states_process(variables, parameters)
  )
  # ======================
  # Mortality step
  # ======================
