...
calculate_tbv_antibodies <- function(t, tbv_iiva, tbv_iivb, tbv_PK_sx, tbv_PK_zscore, age_at_vaccination){ 
  ...
  #W_M now accounts for Stijn's correction factor:
  W_M_F <- c(4.220668,  6.227039,  7.604794,  8.977390, 10.433913, 11.846789, 13.565700, 15.512249,
             16.974509, 18.481326, 20.182022, 22.224880, 24.579420, 
             27.143335, 30.113739, 33.485323, 36.632355, 41.383448, 45.666806, 49.496417, 53.097664, 56.230858)
  ...
  #W_M now accounts for Stijn's correction factor:
  W_M_M <- c(4.603616,  6.738805,  8.167185,  9.552424, 10.984514, 12.334798, 13.906277, 15.684144, 17.210922, 
             18.876246, 20.609110, 22.378811, 24.257314, 26.500129, 
             29.063549, 31.425937, 33.783907, 37.087975, 41.606539, 45.685019, 49.132257, 51.251216)
  ...
  vnapply(
    ...
    function(i) {
      ...
      b <- 0.75
      if(age_agent < (5*365) & age_agent >= (2*365)){
        b <- 0.9
      }
      if(age_agent < (2*365) & age_agent >= 90){
        b <- 1.0
      }
      if(age_agent < 90){
        b <- 1.1
