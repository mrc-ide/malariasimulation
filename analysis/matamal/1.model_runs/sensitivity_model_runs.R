...
#parameter set
df_scenario <- readRDS("analysis/matamal/output/df_scenario_sens.rds")
...
###########for control arm, does not need to be in the loop#############
...
df_scenario_control <- df_scenario %>%
  select(Q0, bites_Bed, retention_net_in, DP_cov_in) %>%
  distinct()

#df_scenario_control <- df_scenario_control[1,]

##CONTROL ARM###

################################################################################
for (i in seq_len(nrow(df_scenario_control))) {
  Q0_i <- df_scenario_control$Q0[i]
  phi_bednets_i <- df_scenario_control$bites_Bed[i]
  retention_i <- df_scenario_control$retention_net_in[i]
  #endec_mu_i <- df_scenario$endec_mu[i]
  #wane_endec_i <- df_scenario$wane_endec[i]
  DP_cov_i <- df_scenario_control$DP_cov_in[i]
  ...
  #DP_cov <- unique(df_setting$DP_cov_in)
  #DP_cov_i <- unique(c(df_setting$lower_DP_cov, df_setting$mean_DP_cov, df_setting$upper_DP_cov))
  
  mdaparams_i <- set_mda(mdaparams_i,
                         drug = 1, #just DP
                         timesteps = mda_events,
                         coverages = rep(DP_cov_i, mda_distr), #modify cov for each distr acc to data
                         min_ages = rep(6*30, length(mda_events)), #min age is 6 months
                         max_ages = rep(80 * 365, length(mda_events))) #the max age of pop
...
names(sim_list) <- paste0(
  "Q0_", df_scenario_control$Q0,
  "_phi_", df_scenario_control$bites_Bed,
  "_net_", df_scenario_control$retention_net_in ,
  "_DP_cov_", df_scenario_control$DP_cov_in
)


results_list <- vector("list", length(sim_list))
names(results_list) <- names(sim_list)

for (i in seq_along(sim_list)) {
  scenario <- names(sim_list)[i]
  message("Running control scenario: ", scenario)
  
  results_list[[i]] <- run_simulation(
    timesteps = sim_length,
    parameters = sim_list[[i]],
    correlations = correlations  # ensure this is correct per scenario
  )
  
  # Save every 5 runs OR final run
  if (i %% 5 == 0 || i == length(sim_list)) { #change to %%50 or, smaller, like 10
    saveRDS(results_list[1:i],
            file.path("analysis/matamal/1.model_runs/sensitivity_output/control", paste0("control_sensitivity_partial_", i, ".rds")))
    message("✅ Saved results up to run ", i)
    gc(verbose = FALSE)
  }
}

partial_files <- list.files(
  "analysis/matamal/1.model_runs/sensitivity_output/control",
  pattern = "control_sensitivity_partial_.*\\.rds$",
  full.names = TRUE
)

# Combine
all_results_control <- do.call(c, lapply(partial_files, readRDS))  # use bind_rows if each element is a data.frame

# Save final combined file
saveRDS(all_results_control, 
        "analysis/matamal/1.model_runs/sensitivity_output/control/control_sensitivity_all.rds")

ggplot(all_results_control$Q0_0.202_phi_0.836_net_360_DP_cov_0.702, aes(x = ((timestep-1)/365)+2012,y = n_detect_pcr_0_29200/n_age_0_29200))+
  geom_line()
...
#saveRDS(run_control_list, file = "analysis/matamal/1.model_runs/sensitivity_output/control/sensitivity_model_runs.rds")
...
#test one row 
df_scenario_int <- df_scenario_int[1,]

sim_list_int <- list()

for (i in seq_len(nrow(df_scenario_int))) {
  Q0_i <- df_scenario_int$Q0[i]
  phi_bednets_i <- df_scenario_int$bites_Bed[i]
  retention_i <- df_scenario_int$retention_net_in[i]
  DP_cov_i <- df_scenario_int$DP_cov_in[i]
  endec_mu_i <- df_scenario_int$mu_endec[i]
  wane_endec_i <- df_scenario_int$wane_endec[i]
  ...
  endec_params_i <- set_endectocide(mdaparams_i, timesteps = early_steps,
                                  endec_on = early_endec_on, 
                                  endec_ts = early_endec_ts)
  
  
  
  correlations <- get_correlation_parameters(endec_params_i)
  correlations$inter_round_rho('bednets', 1) #same people get ITNs each year
  correlations$inter_intervention_rho('bednets', 'mda', -1) #not necessarily same people getting ITNs and DP
  
  
  # store in list
  sim_list_int[[i]] <- endec_params_i
  
}

# assign scenario names after the loop
names(sim_list_int) <- paste0(
...
results_list_int <- vector("list", length(sim_list_int))
names(results_list_int) <- names(sim_list_int)

for (i in seq_along(sim_list_int)) {
  scenario <- names(sim_list_int)[i]
  message("Running intervention scenario: ", scenario)
  
  results_list[[i]] <- run_simulation(
    timesteps = sim_length,
    parameters = sim_list_int[[i]],
    correlations = correlations  # ensure this is correct per scenario
  )
  
  # Save every 10 runs OR final run
  if (i %% 10 == 0 || i == length(sim_list_int)) { #change to %%50 or, smaller, like 10
    saveRDS(results_list_int[1:i],
            file.path("analysis/matamal/1.model_runs/sensitivity_output/intervention", paste0("intervention_sensitivity_partial_", i, ".rds")))
    message("✅ Saved results up to run ", i)
    gc(verbose = FALSE)
  }
}
...
partial_files <- list.files(
  "analysis/matamal/1.model_runs/sensitivity_output/intervention",
  pattern = "intervention_sensitivity_partial_.*\\.rds$",
  full.names = TRUE
)

# Combine
all_results_intervention <- do.call(c, lapply(partial_files, readRDS))  # use bind_rows if each element is a data.frame

# Save final combined file
saveRDS(all_results_intervention, 
        "analysis/matamal/1.model_runs/sensitivity_output/intervention/intervention_sensitivity_all.rds")

#ggplot(all_results_intervention$Q0_0.202_phi_0.836_net_360_DP_cov_0.702, aes(x = ((timestep-1)/365)+2012,y = n_detect_lm_0_29200/n_age_0_29200))+
#  geom_line()


#run again with corrected output format 
#for (i in seq_along(sim_list_int)) {
#  scenario <- names(sim_list_int)[i]
#  message("Running intervention scenario: ", scenario)
#  
...
#  
#  # Save every 10 runs OR final run
#  if (i %% 1 == 0 || i == length(sim_list_int)) { #change to %%50 or, smaller, like 10
#    saveRDS(results_list_int[1:i],
#            file.path("analysis/matamal/1.model_runs/sensitivity_output/intervention2", paste0("intervention_sensitivity_partial_", i, ".rds")))
#    message("✅ Saved results up to run ", i)
#    gc(verbose = FALSE)
#  }
#}
...
## Save final combined file
#saveRDS(all_results_intervention, 
#        "analysis/matamal/1.model_runs/sensitivity_output/intervention2/intervention_sensitivity_all.rds")
