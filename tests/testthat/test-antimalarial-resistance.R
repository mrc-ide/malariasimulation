...
test_that("get_antimalarial_resistance_parameters() retrieves the correct parameters for each individual given the drug they were administered", {
  
  parameters <- get_parameters()
  parameters <- set_drugs(parameters = parameters, drugs = list(SP_AQ_params, AL_params))
  parameters <- set_clinical_treatment(parameters = parameters,
                                       drug = 1,
                                       timesteps = c(0, 10), 
                                       coverages = c(0.1, 0.2))
  ...
  parameters <- set_antimalarial_resistance(parameters = parameters,
                                            drug = 1,
                                            timesteps = c(0, 30),
                                            artemisinin_resistance = c(0.1, 0.2),
                                            partner_drug_resistance = c(0.05, 0.09),
                                            slow_parasite_clearance_prob = c(0.3, 0.6),
                                            early_treatment_failure_prob = c(0.88, 0.91),
                                            late_clinical_failure_prob = c(0.34, 0.54),
                                            late_parasitological_prob = c(0.41, 0.65),
                                            reinfection_prob = c(0.05, 0.01), 
                                            slow_parasite_clearance_time = 5)
  ...
  target_parameters$artemisinin_resistance_proportion <- c(0.1, 0.8, 0.8, 0.1, 0.1)
  target_parameters$partner_drug_resistance_proportion <- c(0.05, 0.43, 0.43, 0.05, 0.05)
  target_parameters$slow_parasite_clearance_probability <- c(0.3, 0.4, 0.4, 0.3, 0.3)
  target_parameters$early_treatment_failure_probability <- c(0.88, 0.45, 0.45, 0.88, 0.88)
  target_parameters$late_clinical_failure_probability <- c(0.34, 0.01, 0.01, 0.34, 0.34)
  target_parameters$late_parasitological_failure_probability <- c(0.41, 0.06, 0.06, 0.41, 0.41)
  target_parameters$reinfection_during_prophylaxis_probability <- c(0.05, 0.86, 0.86, 0.05, 0.05)
  target_parameters$dt_slow_parasite_clearance <- c(5, 10, 10, 5, 5)
  
  expect_identical(object = resistance_parameters, expected = target_parameters)
