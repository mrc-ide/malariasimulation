test_that('Test age parameter function works', {
  ...
  parameters_f <- set_epi_outputs(parameters_f,
                                  ...
                                  clinical_incidence = age_limits+1,
                                  severe_incidence = age_limits+2,
                                  prevalence = age_limits+3
  )
  parameters_v <- set_epi_outputs(parameters_v,
                  age_group = age_limits,
                  patent_incidence = age_limits+1,
                  clinical_incidence = age_limits+2,
                  prevalence = age_limits+3,
                  n_with_hypnozoites = age_limits+4
                  )
  ...
  expect_true(
    all(
      paste0(rep(c("n_age",
             ...
             age_limits[-4]+rep(c(0,rep(c(1,2), each = 3), rep(3,4)), each = 3),"_",age_limits[-1]-1+rep(c(0,rep(c(1,2), each = 3), rep(3,4)), each = 3)
  ) %in%
    names(sim_f)))
  ...
  expect_true(all(paste0(rep(c("n_age",
               ...
               "n","n_with_hypnozoites"), each = 3),"_",
         age_limits[-4]+rep(c(0,rep(c(1,2,3), each = 3), rep(4,2)), each = 3),"_",age_limits[-1]-1+rep(c(0,rep(c(1,2,3), each = 3), rep(4,2)), each = 3)) %in%
    names(sim_v)))
