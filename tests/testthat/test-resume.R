...
test_that("TBV intervention can be added when resuming", {
  ...
  test_resume(
    timesteps = 200,
    warmup_parameters = base %>% set_tbv(timesteps=25, coverage=1, ages=5:60),
...
test_that("Bednets intervention can be added when resuming", {
  set_default_bednets <- function(parameters, timesteps, retention) {
    n <- length(timesteps)
    set_bednets(
      parameters,
      timesteps = timesteps,
  ...
  test_resume(
    ...
    parameters = base %>% set_default_bednets(25, 100))
})
