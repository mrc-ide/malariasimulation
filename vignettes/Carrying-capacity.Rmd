...
carrying capacity. The helper function `set_larval_source_management()`
...
`set_rescaled_carrying_capacity()` and
`set_flexible_carrying_capacity()` give complete freedom to allow
relative or absolute changes to the carrying capacity to be made. Here
...
p_stephensi <- p_stephensi |>
  set_rescaled_carrying_capacity(
    timesteps = 365,
    scalers = matrix(c(1, 2000), ncol = 2)
  )
...
The functions `set_rescaled_carrying_capacity()` and
`set_flexible_carrying_capacity()` can be used to modify or set very
