...
# The cali and mgcv packages are required but not loaded. To run the code manually load these:
# packages.
#library(mgcv)
#library(cali)
...
As a final exercise, let's compare graphically the `calibrate()` approach with the `malariasimulation` method when the same parameter set is used. The plot below shows the change in *Pf*PR~2-10~ over the duration of the simulation under the initial EIR values recommended by the a) `cali` and b) `malariasimulation` methods to achieve the target *Pf*PR~2-10~ value of 0.3. The blue horizontal line represents the target *Pf*PR~2-10~ and the black lines either side the upper and lower tolerance limits specified for the `cali` method.
