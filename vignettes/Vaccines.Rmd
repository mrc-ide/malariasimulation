...
plot_inci <- function(xtext = 1 - 0.1, ytext = 4){
  ...
  lines(output$time_year, predict(curve_values),
        col = cols[6],
        lwd = 3)
  text(x = xtext, y = ytext, 'Vaccination',
       cex = 1, col = cols[1], srt = 90)
}

# Plot parasite prevalence
plot_prev <- function(xtext = 1 - 0.1, ytext = 0.44){
  output$time_year <- output$timestep / year
...
You can look at the distribution of doses using the `n_rtss_mass_dose\_\*` outputs. It is always a good idea to check the timing of vaccine doses to ensure that it is specified as intended.
