...
plot_doses <- function(){
  if(any(names(output) == 'n_rtss_epi_dose_1')) {
    ...
    barplot(doses$n_rtss_epi_dose_1, doses$month, col = cols[6],
              xlab = 'Time (months)', ylab = 'Doses (n)')
    barplot(doses$n_rtss_epi_dose_2, doses$month, add = TRUE, col = cols[7])
    barplot(doses$n_rtss_epi_dose_3, doses$month, add = TRUE, col = cols[8])
    barplot(doses$n_rtss_epi_booster_1, doses$month, add = TRUE, col = cols[5])
    ...
    doses <- output |> 
      ...
      barplot(doses$n_rtss_mass_dose_1,doses$month, col = cols[6],
              xlab = 'Time (months)', ylab = 'Doses (n)')
      barplot(doses$n_rtss_mass_dose_2, doses$month, add = TRUE, col = cols[7])
      barplot(doses$n_rtss_mass_dose_3, doses$month, add = TRUE, col = cols[8])
      barplot(doses$n_rtss_mass_booster_1, doses$month, add = TRUE, col = cols[5])
      
  }
  
  
  plot(x = doses$month, y = doses$count, type = 'h', 
       lwd = 10, lend=1, ylim = c(0,max(doses$count + 500)),
       col = c(cols[6], cols[7],cols[8],cols[5]),
       xlab = 'Time (months)', ylab = 'Doses (n)',
       xaxs = "i", yaxs = "i", bty = "l")
  text(x = max(doses$month[doses$count!=0])-1, y =500, 'Booster',
