...
plot_inci <- function(type = "not seasonal"){
  ...
  plot(x = output$time_year, y = output$clinical_incidence,
       ...
       ylim = c(0, max(output$clinical_incidence, na.rm = T)*1.3),
       xaxs = "i", yaxs = "i")
  grid(lty = 2, col = "grey80", lwd = 0.5)
  abline(v = vaccinetime, col = "black", lty = 2, lwd = 2.5)
  text(x = vaccinetime + 0.05, y = max(output$clinical_incidence, na.rm = T)*1.2, labels = "Start of\nvaccination", adj = 0, cex = 0.8)
  curve_values <- loess(clinical_incidence ~ time_year, data = output,
                        span = 0.3, method = "loess") 
  lines(output$time_year, predict(curve_values, newdata = output$time_year),
        col = cols[5], lwd = 3)
  curve_valuescomp <- loess(clinical_incidence ~ time_year, data = comparison, span = 0.3, 
                            method = "loess") 
  lines(comparison$time_year, predict(curve_valuescomp, newdata = comparison$time_year), 
        col = cols[6], lwd = 3)
...
# Plot doses
output$month <- ceiling(output$timestep / month)
doses <- output[, c(grep("n_pev" , names(output), value = T), "month")]
