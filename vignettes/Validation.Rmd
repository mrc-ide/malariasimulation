...
agreement_plots <- lapply(
  ...
  function(i) {
    ...
    equilibrium <- data.frame(
      ...
      S_count = rep(state_props[['S']] * pop_size, sim_length),
      D_count = rep(state_props[['D']] * pop_size, sim_length),
      A_count = rep(state_props[['A']] * pop_size, sim_length),
      U_count = rep(state_props[['U']] * pop_size, sim_length)
    )
    ggplot(
      melt(
        output[c(
...
immunity_plots <- lapply(
  ...
  function(i) {
    ...
    equilibrium <- data.frame(
      timestep = seq(sim_length),
      ica_mean = rep(weighted.mean(eq$states[, 'ICA'], w), sim_length),
      icm_mean = rep(weighted.mean(eq$states[, 'ICM'], w), sim_length),
      ib_mean = rep(weighted.mean(eq$states[, 'IB'], w), sim_length),
      id_mean = rep(weighted.mean(eq$states[, 'ID'], w), sim_length)
    )
