...
for (density in c(5)) {
  ...
  simparams$ub <- params$ub - 1
  simparams$uc <- params$uc - 1
  simparams$ud <- params$ud - 1
  EIR <- estimate_eir()
  if (EIR > 0) {
    ...
    simparams$init_ica <- eq$states[,'ICA']
    simparams$init_icm <- eq$states[,'ICM']
    simparams$init_ib <- eq$states[,'IB']
    simparams$init_id <- eq$states[,'ID']
