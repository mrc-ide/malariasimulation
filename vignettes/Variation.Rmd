...
plot(x = df$timestep, y = df$median, 
  ...
  ggplot(df)+
    geom_line(aes(x = timestep, y = median, group = 1))+
    geom_ribbon(aes(x = timestep, ymin = lowq, ymax = highq), alpha = .2)+
    xlab('timestep') + ylab('Cases detected in children aged 2-10 years') + ggtitle('IQR spread')
  ggplot(df)+
    geom_line(aes(x = timestep, y = mean, group = 1))+
    geom_ribbon(aes(x = timestep, ymin = lowci, ymax = highci), alpha = .2)+
    xlab('timestep') + ylab('Cases detected in children aged 2-10 years') + ggtitle('95% confidence interval')
