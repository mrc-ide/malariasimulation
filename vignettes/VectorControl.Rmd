...
This vignette explains how to implement nets (ITNs or LLINs), indoor spraying of residual insecticides (IRS) and larval source or habitat management (LSM).
...
simparams <- get_parameters(
  list(
    ...
    # For habitat management we must set individual_mosquitoes = FALSE
...
efficacy and use characteristics of the bed nets on each distribution...
...
lsm_new_eqm: a value of 1 is no change, 0 simulates full reduction in recruitment to adult mosquitoes from pupae.

lsm_rate_alpha: the default is -4 and it is recommended to keep this estimate fixed. But this can vary between -6 and 5. The positive estimates take longer to reach the new level of adult recruitment (the proportional reduction as set by lsm_new_eqm)

lsm_rate_beta: the default is 0.1 which achieves the reduced recruitment equilibrium (the proportion of mosquitoes recruited to the susceptible cohort as defined by lsm_new_eqm) after about 30 days (if lsm_rate_alpha is -4). The range to try must be positive, recommended to keep this between 0.01 and 0.5 
...
habitat_managementparams <- set_habitat_management(
  ...
  habitat_management_timesteps = lsm_events$timestep,
  lsm_new_eqm = matrix(0.5,nrow=1,ncol=3),
  lsm_rate_alpha = matrix(-4,nrow=1,ncol=3),
  lsm_rate_beta = matrix(0.1,nrow=1,ncol=3)
)
