...
add_intervention_lines <- function(plot, events) {
  plot
  abline(v = events$timestep, col = cols[6])
...
```{r}
plot(x = output$timestep, y = output$n_use_net, type = 'l', col = '',
...
See the vignettes from the `netz` package: 
* [here](https://mrc-ide.github.io/netz/articles/overview.html) that explains how to estimate the distribution coverage from a specified usage. 
* [here](https://mrc-ide.github.io/netz/articles/population_usage.html) that explains how to estimate population usage from input distribution. 
```{r}
