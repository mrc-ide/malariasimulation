...
The parameter values for pyrethroid-only and pyrethroid-PBO nets at various resistance levels can be found in Table S1.3 in the Supplementary Appendix 2 of [Sherrard-Smith, et al., 2022](https://doi.org/10.1016/S2542-5196(21)00296-5). Note that in malariasimulation the parameter gamman, which governs the rate of decay of the killing activity of the active ingredient (or ingredients) in the nets, is sometimes termed a half-life. Here, however, it is the mean duration of the killing activity of the active ingredient(s). To convert from a half-life it is necessary to divide the number by log(2) (approximately equal to 0.69315), to obtain the rate parameter. Note that this conversion factor should be used when entering values from Table S1.3 of Sherrard-Smith, et al., (2022). 
...
bednetparams <- set_bednets(
  ...
  gamman = rep(2.226 * 365, 2) # Vector of the mean duration of the killing activity of the active ingredient(s) in the bed nets, for each distribution timestep
)
