...
Indoor residual spraying (IRS) is another common intervention to prevent malaria. We can add IRS in the model with different timings and coverages and in settings with specified proportions of mosquito species with the help of the `set_spraying()` function. 
...
plot_prev <- function() {
  plot(x = output$timestep, y = output$n_detect_730_3650/output$n_730_3650, 
       ...
       xaxs = "i", yaxs = "i", bty = "l", ylim = c(0.05, 0.9))
  lines(x = output_control$timestep, y = output_control$n_detect_730_3650/output_control$n_730_3650,
        col = cols[6], lwd = 2.5)
  abline(v = sprayingtimesteps, lty = 2, lwd = 2.5, col = cols[1])
  grid(lty = 2, col = 'grey80', lwd = 1)
  legend('topright', box.lty = 0, legend = c('Spraying', 'Prevalence for IRS scenario','Prevalence for control scenario'),
